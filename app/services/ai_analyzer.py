...
Bloque 3.2 — Cambios respecto al bloque anterior:
  - analyze() acepta un `system_prompt` opcional.
    Si se pasa (viene del DocClassifier), se usa ese prompt especializado.
    Si no se pasa, usa el prompt genérico original (retrocompatible).
  - _run_analysis() construye el prompt dinámicamente.
"""
